...
var KlistProvider = {
     ...
     * @param {DataCallback} cb - with lists as data
     */
     ...
     * @param {String} id - the list identifier
     * @param {DataCallback} cb - with list as data
     */
    getList : function(id, cb){
        client.hgetall(keys.list + id, function(err, list){
            if(err){
                return cb(new Error('Error while retrieving list ' +  id + ' : ' + err));
            }
            cb(null, list);
        });
    },
