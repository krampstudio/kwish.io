...
var UserProvider = {
    ...
    /**
     * Remove the auth token of a user
     * @param {String} login - the user login
     * @param {ConfirmCallback} cb - true if removed
     */
    removeToken : function(login, cb){
        client.del(keys.token + login, function(err, reply){
            if(err){
                return cb(new Error('Error while removing token for ' + login +  ' : '  + err));
            }
